...
% Initialization.
salthresh = 0.3;            % Initial salience value for reward (used in 'relhisal' reinforcment).
        ...
        if (mod(sec,testint)==0) 
            ...
            if t==1000 % Based on the 1 s timeseries of smoothed summed motor neuron spikes, generate a sound.
                ...
                if ~strcmp(reinforcer,'range')
                    ...
                    %%%%%%Vivian: add the python code here
                    if strcmp(yoke,'false')                  
                        reward = py.classify.classify([wavdir,'/synth_',id,'_',num2str(sec),'.wav']);
                    elseif strcmp(yoke,'true')
                        display([wavdir,'/synth_',id,'_yoke_',num2str(sec),'.wav']);
                        reward = py.classify.classify([wavdir,'/synth_',id,'_yoke_',num2str(sec),'.wav']);
                    end
           
    
                    if ~(strcmp(reinforcer,'human') && strcmp(yoke, 'false'))
                        display(['p(reward) = ',num2str(reward)]);
                    end
                end
                ...
                elseif strcmp(reinforcer,'classify')
                    ...
                    if reward >salthresh
                        ...
                        if mean(temprewhist)>=.7 %previously it was 30%
                            salthresh=salthresh+.01; % not sure!
